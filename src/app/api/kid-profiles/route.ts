...
export async function GET() {
  ...
  const { data, error } = await sb
    .from('kid_profiles')
    .select('id, school_id, age_range, ordinal, school:schools(name, district, type)')
    .eq('user_id', user.id)
...
export async function POST(req: Request) {
  ...
  const { data, error } = await sb.from('kid_profiles').insert(rows).select('id, school_id, age_range, ordinal');
  if (error) return NextResponse.json({ error: 'db_error', detail: error.message }, { status: 500 });
